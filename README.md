...
and transition: board cards with 4-level spaced-repetition grading, an Army
dictionary, a doctrine library, branching leadership scenarios, and a DA 4856
exporter that fills the real form.

What the app holds today. This table is generated from the content bank by
`node tools/content-manifest.mjs --write` (run from `guidon-app/`) and checked
by `npm run lint:patterns` - a count typed into a document by hand is a count
that will be wrong:
