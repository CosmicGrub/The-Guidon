...
import { renderSource } from "./citation-parse.mjs";   // a card's source is a structured array since ROADMAP item F Wave 2
...
const bq1 = real.data.board && Array.isArray(real.data.board.questions)
  ...
  : null;
bq1 && bq1.q === "What are the seven Army Values?" && renderSource(bq1.source) === "ADP 6-22"
  ? ok('known card "bq1" (Army Values, ADP 6-22) survived parsing intact')
  : bad("known card bq1 missing or changed: " + JSON.stringify(bq1));

// Exact, not "more than 900": the content manifest records how many cards the
