...
/** The shell scripts tools/build.mjs splices into web/ (not sw.js: it runs
 *  in the service worker, where window.G does not exist). */
export const SHELL_SCRIPTS = ["pdf-defer.js", "pdfjs-defer.js", "native.js", "xwin.js", "room-tauri.js", "room-web.js", "notify.js", "biometric.js", "pwa.js"];
...
/** index.html with each <script> block put through blankSource; the markup,
 *  the stylesheet and every offset are left exactly as they were. */
export function blankHtmlScripts(html) {
  ...
  for (const [start, end] of blocks) {
    ...
    text += html.slice(at, start) + b.text; code += html.slice(at, start) + b.code;
    at = end;
  }
  return { text: text + html.slice(at), code: code + html.slice(at), blocks };
}
 ...
 *   if-call     if (G.a) G.a.b(...)
 *   optional    G.a?.b(...)   G.a.b?.(...)
 * Not seen: a guard written through an alias (var db = G.db; typeof db.put),
 * a guard and its call on different lines for the and-call / if-call shapes,
 * and guards on data (G.a && G.a.value) - there is no call to go missing.
 */
export function collectGuards(sources) {
  ...
  for (const s of sources) {
    ...
    for (const { line, no } of codeLines(s.code)) {
      ...
      for (const m of line.matchAll(ifRe)) { const a = "G" + m[1], b = "G" + m[2]; if (b.startsWith(a + ".")) push(b, "if-call"); }
