...
/* ======================================================================
   (e) Every route in ROUTES has a matching entry in DEMO_NOTES. The Guided
   ...
   ====================================================================== */
{
  ...
  if (routesDeclIdx === -1 || notesDeclIdx === -1) {
    bad("(e) could not locate ROUTES and/or DEMO_NOTES declarations");
  } else {
    ...
    if (routesBody == null || notesBody == null) {
      bad("(e) could not extract the balanced ROUTES/DEMO_NOTES body");
    } else {
      ...
      if (missing.length) {
        bad(`(e) ${missing.length} route(s) with no matching DEMO_NOTES entry - the Guided Tour's Highlights/full tracks will fall back to generic copy for: ${missing.join(", ")}`);
      } else {
        ok(`(e) every route has a matching DEMO_NOTES entry (${routeHashes.length} routes, ${EXCLUDED.size} intentionally excluded)`);
      }
    }
  }
}
