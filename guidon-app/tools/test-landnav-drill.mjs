...
// The "New" badges this drill carried for v1.9.0 (the #/drills menu card and
// the "Grid plot" mode button) were retired in v1.12.1: one wave is live at
// a time, and a badge that outlives three releases stops meaning anything.
// The generic `since` support stays in both places for the next wave, so
// what is checked here is that neither still claims to be new.
...
landnavCardInfo && landnavCardInfo.ariaLabel === "Land Navigation practice drill" && landnavCardInfo.badgeText === null
  ? ok('the #/drills menu card no longer carries the retired v1.9.0 "New" badge (plain accessible name, no .badge.green)')
  : bad("Land Navigation menu card badge info: " + JSON.stringify(landnavCardInfo));

await openLandNavDrill();
// aria-label, not textContent: a mode button that carries a `since` badge
// renders it as a child node, so the accessible name is the exact, stable
// signal for what each button IS.
...
JSON.stringify(modeButtons.slice(0, 3)) === JSON.stringify(["Back azimuth", "Pace count", "Grid coordinate"])
  ? ok('Land Navigation drill opens on the mode switch: "Back azimuth" / "Pace count" / "Grid coordinate"')
  : bad("mode switch buttons: " + JSON.stringify(modeButtons));
modeButtons[3] === "Grid plot"
  ? ok('a 4th mode, "Grid plot", is present alongside the original three, with a plain accessible name')
  : bad("4th mode button: " + JSON.stringify(modeButtons[3]));
const plotBadges = await page.locator(".panel .segmented button .badge.green").count();
plotBadges === 0
  ? ok('no mode button carries a "New" badge now that the v1.9.0 wave is retired')
  : bad("mode buttons still carrying a New badge: " + plotBadges);
