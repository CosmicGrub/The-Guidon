...
**Shipped from the old "Later" list (v1.17.0):** data-driven What's New; the legal package as a verified, version-stamped artifact; the macOS launch proof, the optional fixed-name Mac file and the notarization path (the in-app Mac button stays a page link until the owner flips `MAC_DIRECT_LINK`); Study Rooms carrying PT plans and Team Training sessions; lanes on the ESP32 handheld (compiles and host-tests in CI; deck picker still to be tried on the physical device).

**Later:** UNITPACKS_PLACEHOLDER Board Simulator run history rolled up into the Readiness screen; a leader-side results view for Team Training; the owner-only items below.
