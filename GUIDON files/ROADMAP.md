...
**Current version:** v1.17.0 (guidon-app/package.json), 2026-09-26 — per-unit study decks (a unit leader builds a small, screened, data-only deck; a Soldier adds it from Settings -> Study Preferences -> Unit decks and studies it in Board Drill, Quiz, Rapid Fire and Search); Study Rooms can hand a PT plan or a Team Training session (not just a deck) to the seated devices in a room, receiver-confirmed and screened on both ends; Team Training gained saved sessions; Collective Decision is a native `G.engine` mode (ROADMAP 3g item H closed); every board card and every study-section citation is a structured `{pub, edition, para, quoteKind}` record with one renderer and one lint (3g item F Waves 2 and 3 closed), with four pending-source cards honestly relabelled as study-guide answers; the ESP32 handheld can carry MOS decks as selectable lanes (firmware CI now gates `CI green`; not yet flashed to hardware); the ABCP/BRS and NCOPDS pin-on accuracy fixes; the legal package as a stamped, verified artifact; and What's New as data (see CHANGELOG.md's v1.17.0 entry). v1.16.0 (2026-09-23) — the holistic follow-up release: Global Search now indexes the Dictionary plus Forms/Counsel/Develop/Health with real relevance ranking; the Fitness screen scores a real Army Fitness Test from raw per-event performance against the official tables; two new content packs (68W Combat Medic MOS deck, Master Leader Course prep + 41 E7/E8 board cards - the app's first study material above E6); PRT Drill now covers all five standard drills, not just Preparation; the NCOER (DA 2166-9-2) worksheet gained its missing Senior Rater section and a rating-scale fix; Board Drill category pinning plus nav-personalization backup/restore; and a fixed release-asset fan-out (see CHANGELOG.md's v1.16.0 entry). v1.15.7 (2026-09-23) — Board Drill's pillar/category/regulation filters consolidated into a real hierarchy instead of three always-visible, disconnected chip rows (category narrows to the active pillar, regulation narrows to pillar+category and collapses behind a disclosure by default); fullscreen study (theater mode) now grows into place on entry instead of snapping, including the automatic Fold5-folded-landscape trigger (see CHANGELOG.md's v1.15.7 entry). v1.15.6 (2026-09-23) — Nav Spotlight: a shared `G.priorities.rank()` engine surfaces onboarding priorities at the top of the sidebar without ever hiding anything (off by default, offered at onboarding, editable from Settings, one tap to turn off); MOI Import now recognizes Army Directive citations, abbreviated or spelled out (see CHANGELOG.md's v1.15.6 entry). v1.15.5 (2026-09-22) — `views.creeds`'s source line no longer shows a literal "— undefined" suffix for a citation with no `status` field (found via code review, see CHANGELOG.md's v1.15.5 entry). v1.15.4 (2026-09-22) shipped ROADMAP 3g item I complete: `#/drills`' PRT session builder now reads its Preparation Drill exercise names, session composition, and pending-drill status through `store.prtMeta()` instead of hand-maintained duplicate data. v1.15.3 (2026-09-22) shipped ROADMAP 3g item H's completion contract: `G.engine.run()`'s solo path now reports a real `{completed, cancelled, scenarioId}` signal, correctly derived from session state and dispatched at most once; Board Simulator's phase registry turned out already real; folding Collective mode into the core engine's own dispatch (today a runtime wrapper, not a render mode) remains open, its own separate round (see CHANGELOG.md's v1.15.3 entry). v1.15.0 (2026-09-22) shipped ROADMAP item F Wave 1: doctrine, creeds, PRT, and scenario doctrine citations moved to a real `{pub, edition, para, quoteKind}[]` structured schema (see §3g item F below and CHANGELOG.md for the full accounting); board.questions and the remaining free-text sections are Waves 2-3, not yet started. v1.14.2 (2026-09-22) shipped Global Search now covers screens and Settings sub-sections, not just content (a new "screen" search type; searching "dark mode", "PCS", or "TSP" now lands on Theme, Assignments, or Money). v1.14.1 (2026-09-22) was a one-line Search fix (MOI Import now appears in the unindexed-domains fallback instead of returning zero results). v1.14.0 (2026-09-21) shipped a PT Planner custom session composer, ROADMAP 3g "customizable screen layouts" (a shared date-grid layout plus one screen-specific layout each for PT Planner and Career Calendar, six new themes), and a full MOI Import overhaul (multi-plan history with diffing, match-tier badges, board-date awareness, an opt-in per-plan-family topic filter, UCMJ Article citation recognition, and a leader-side dates-only squad briefing tracker). v1.13.0 (2026-09-20) shipped the Cybersecurity Fundamentals module, the 2026-09 OPSEC/legal audit's safe fixes, a reachable backup opt-in for the roster and Risk Worksheets, and ROADMAP 3g Wave 1 (module manifest, storage contract, shared test kit, generated content floors). v1.12.1 (2026-09-19) was the prior audit-fix release: 54 confirmed defects from the 2026-09-15..18 merges fixed, runtime content packs brought under the same lints as the seed. v1.11.0 and v1.12.0 were prepared but never released and are superseded; v1.10.1 was published retroactively. One immutable tag feeds web/PWA + standalone, Android (signed on the owner's machine - see guidon-app/docs/release-runbook.md), Windows, macOS, iOS Simulator parity, and the ESP32 firmware/content fork. See CHANGELOG.md for distribution boundaries.
...
Source: `GUIDON files/AUDIT-2026-09.md` section 6 (the audit of PRs #172-#183 and the v1.12.1 fix release). The new tools - Board Simulator, Team Training, PT Planner, the Cybersecurity & OPSEC curriculum, the 92A deck, Study Rooms mixed decks, My unit - arrived as runtime modules that wrap core functions and push content into the seed at load. v1.12.1 put lints around that (`tools/assemble-bank.mjs`, `tools/lint-content-packs.mjs`); the items below remove the need for the workaround. Items A-J below have all shipped (see each item); what is left is listed under "Later". Each was sized to land green on its own.
...
- F. Structured citations (`{ pub, edition, para, quoteKind }`) so "verbatim" headings, regulation chips, the superseded-publication check and the rights gate are mechanical. **Wave 1 SHIPPED (v1.15.0):** doctrine.entries, creeds, PRT, and scenario `doctrine[]` - 650 sites, 30 consumers, a new `tools/lint-citation-schema.mjs` gate. **Wave 2 SHIPPED (v1.17.0):** all 1,347 board cards (997 static + 350 from 13 content packs) carry `source: [{pub, edition, para, quoteKind, ...}]`, the card-level `verbatim` flag is folded into `quoteKind`, `G.board.regulationsOf()` derives from the structured source, and content packs write `ctx.cite(text, quoteKind)`; the original sizing note follows. board.questions' 997 free-text sources - the largest pool, coupled to `G.board.regulationsOf()`'s existing runtime regex parser (a deliberate prior decision not to persist a structured field, per that function's own header comment) and a separate `q.verbatim` boolean that needs folding into the new `quoteKind` enum; real added complexity beyond Wave 1's rename-only shape, sized as its own round. **Wave 3 SHIPPED (v1.17.0):** curriculum/counsel/counsel_bullets/forms/idp/resilience/finance (plus transition, writing and PRT cadence overrides) - 687 records on the same shape, one renderer (`renderCitation` / `G.util.citeText`), one lint (`tools/lint-citation-schema.mjs`) over the assembled bank. Open follow-ups: eight forms name their citation `ref` (never shown; renaming adds a chip, search hits and MOI counts - a product call), and board `para` keeps its locator word ("para 3-9c") while Wave 1 `para` is bare ("1-34a"), to be reconciled before deep links.
...
**Shipped from the old "Later" list (v1.17.0):** data-driven What's New; the legal package as a verified, version-stamped artifact; the macOS launch proof, the optional fixed-name Mac file and the notarization path (the in-app Mac button stays a page link until the owner flips `MAC_DIRECT_LINK`); Study Rooms carrying PT plans and Team Training sessions; lanes on the ESP32 handheld (compiles and host-tests in CI; deck picker still to be tried on the physical device); per-unit content packs, built as opt-in "Unit decks" on the device (Settings -> Study Preferences) rather than through the build-time content-pack API, so a unit's cards can never enter the shipped bank, the ESP32 export or Study Rooms (see `guidon-app/docs/unit-packs.md`).

**Later:** Board Simulator run history rolled up into the Readiness screen; a leader-side results view for Team Training; the owner-only items below.
...
### MOI Import, Phase 2/3 — RESOLVED, shipped as "MOI Scope" (v1.14.0)
Superseded by an opt-in, per-plan-family version that addresses the exact risk this section originally flagged. `moiScopeFamilies` (Settings) narrows `#/board`/`#/doctrine` to the union of every opted-in family's topics — hidden/off by default (`G.mosDecks.activeCodes()`'s own contract, not a forced hide on an uncertain match), so a Soldier only ever sees a narrowed pool they deliberately chose, never a confidently-empty section they didn't ask for. `#/train` is deliberately still untouched (scenario citations remain too generic to filter meaningfully by — the reasoning below still holds there). The underlying citation-collision risk this section warned about (`AR 600-8-2` vs `AR 600-8-22`, `TC 3-21.5` vs `TC 3-21.8`) is unchanged and still why `matchCitation()` has no fuzzy fallback anywhere — MOI Scope only ever narrows by an *already-exact* match's own topic, never a guessed one.
