...
- H. ~~A real `G.engine.run()` completion contract; Collective mode as an engine render mode; Board Simulator on a phase registry.~~ **SHIPPED (v1.15.3).** Investigating this item found two of its three bullets already substantively done and this bullet simply never updated: Collective mode has existed since v1.12.0 (`src/app-modules/00-roadmap-bootstrap.js`'s `G.engine.run` wrapper), and Board Simulator already runs off a real `STEPS` data registry, not hardcoded branches. The real gap - the solo (non-Collective) engine path never told its `onExit` callback whether a session actually completed, forcing Board Simulator to build its own attempt-log side-channel workaround to guess - is fixed: `G.engine.run()`'s solo path now reports the same `{completed, cancelled, scenarioId}` shape Collective's wrapper already used, at all three of its real exit points. See CHANGELOG.md's v1.15.3 entry for the full accounting.
