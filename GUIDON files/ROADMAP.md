...
- **This file's version line and §1 table are still missing the gap between round 8 and round 9** (v1.6.0, v1.7.0 itself, the ESP32 Flashcard OS firmware fork, and PRs #120–#137 all landed with no corresponding entry here) — the Content expansion (Milestones 1–3), round 9, the Nav & Adaptive-Layout overhaul, and round 10 have all now been backfilled with real entries as of this pass, but that one earlier gap still hasn't been. Worth a dedicated pass; not attempted here to keep this edit scoped to the work that just landed.
...
- **`G.modal`'s `dialog()` and `WhatsNew.show()` hand-roll their own Escape/Tab focus-trap instead of reusing `util.modalTrap`** (found round 11, deliberately not fixed that round): duplicated logic, and a narrower Tab-selector than the shared `FOCUSABLE` constant. Currently harmless — neither dialog's content today includes anything (disabled control, link, select, textarea) that would expose the narrower selector — but a real structural refactor of both dialogs' open/close/focus-restore behavior, not a quick fix, so it was deferred rather than rushed. Worth closing before either dialog's content grows.
- **DA Form 4856's mode-switch buttons (Step-by-step / Single form) go visually stale after the first toggle within a modal session** (found round 11 while fixing an adjacent `aria-pressed` gap, out of that bucket's scope): the click handler only redraws the wizard stage and never re-syncs the two buttons' own `classList.active` (or the `aria-pressed` round 11 added, which intentionally mirrors the same set-once-at-creation limitation rather than silently diverging from the visual state). A second or third toggle in the same session leaves the wrong button looking active. Worth a small follow-up bucket.
