...
> **Correction (intuitivism pass, 2026-08-20):** the "~720×1152 portrait" figure above was a researched estimate, not a measurement — it assumed this device class reports DPR 2 to the browser. It doesn't. With the actual Tab S9 FE 5G (SM-X518U) connected via adb this session, `tools/cdp.mjs` attached directly to the real installed app's WebView and read `window.innerWidth` off the live page: **823** in portrait, with `window.devicePixelRatio` actually **1.75**, not 2 (2304×1440 physical ÷ 1.75 ≈ 1317×823, not ÷2 = 1152×720). The true portrait CSS viewport is **~823×1317**, not 720×1152. This also means the "portrait → the compact 600–859px tier" line in the Verification-performed paragraph above was itself a downstream consequence of the same bad DPR assumption, not a separate finding — on the real 823px width it was *already* eligible for the ≥860px tier's neighbor once that boundary moved (see the intuitivism pass, Tier 1(h): the desktop side-rail threshold moved to ≥800px specifically because of this). `tools/verify.mjs`'s `tabS9-portrait` fixture is corrected to 823×1317 to match. Only width and DPR were directly CDP-measured off the live app this session; the 1317 height is the physical/DPR division, corroborated by (not independently re-derived from) the system's own rotation report for the swapped orientation (`w1317dp h823dp`) — a second attempt to reattach and read `window.innerHeight` directly hung mid-session and wasn't retried, so treat 1317 as well-supported rather than device-measured if it matters for future work. Landscape itself was not re-verified the same way (the installed app is orientation-locked to portrait, so CDP couldn't observe it directly there); that figure may also be off but wasn't load-bearing for anything this pass touched and is left as an open item rather than a second inference-based "fix."
...
## 61. Two Media Queries That Quietly Stopped Agreeing With Each Other (session 52, v1.4.6)

The ask this time was explicitly broader than any single bug class: responsive and clean across every layout, device format, configuration, orientation. Where §57-60 chased contrast and legibility, this pass asked a different question of the same app - not "is this text readable" but "does this control work, reliably, at the size and shape the device actually is."

**Two hardware findings earned their place by being investigated and then correctly set aside.** The Fold's unfolded inner screen showed content disappearing under a persistent bottom bar - the instinct is to reach for a safe-area fix. Instead: check first whether the OS renders it, not the app. It showed on the home screen too. That single check turned what could have been a wasted rebuild-and-reship cycle chasing a Samsung multitasking taskbar into a two-line note: not our bug, not our control. The Fold's cover screen rotated to an unusual 2.56:1 landscape got the same treatment - measured, looked correct, left alone. Neither finding cost the pipeline a single line of code, which is exactly the point: the discipline that matters most is knowing when NOT to fix something.

**The real bug was architectural, not cosmetic.** `@media (pointer:coarse)` — the rule meant to guarantee touch targets on ANY touchscreen, regardless of viewport width — had quietly become a strict subset of `@media (max-width:640px)`, the narrow-phone rule. Nobody removed anything to cause this; the width-based rule simply grew over many sessions (each new component's touch-target need patched into the rule a developer was already looking at) while the pointer-based rule stood still. The result: a tablet, an unfolded foldable in landscape, or a touchscreen laptop — all touch-capable, all wider than 640px — silently lost the accessibility floor on more than a dozen interactive classes. This is worth naming as a pattern, not just a bug: two rules that are SUPPOSED to enforce the same guarantee via different conditions will drift apart if only one of them is in view when new components get added. The fix wasn't clever - align both lists to the same comprehensive set - but finding it required treating "why does pointer:coarse only cover five classes when 640px covers fourteen" as a question worth asking, not an assumed-intentional asymmetry.

**One spot-check before trusting 37 findings.** `.search-chip`'s reported 22-24px height looked like it should contradict `button.chip` already sitting in the touch-target list — enough of an apparent inconsistency that a direct measurement was worth the five minutes before touching any code. It confirmed the finding exactly: `pointer:coarse` never had `button.chip` at all, only `max-width:640px` did. The audit was right; my own recollection of the codebase was stale. Small, but it's the same habit as §51's "distrust the measurement, not the code" — applied here in the opposite direction, confirming rather than debunking, which matters just as much for not second-guessing good work.

**One finding stood out for what it sits next to.** `.res-copy-btn` — the copy-to-clipboard control beside the 988 Veterans Crisis Line number — measured roughly 20px tall with zero vertical padding, the smallest interactive control this audit found, on the one screen where a fumbled tap matters most. Flagged high severity by the audit, fixed the same way as everything else: added to the shared touch-target selector list. No special-casing needed once the underlying gap was closed correctly.

**Every fix was re-measured, not trusted from the audit output alone** — the same standing rule since §51's three tooling bugs. One fix needed a second pass on its own: `.topbar-search-btn`'s width override didn't take on the first attempt, because a later, unconditional rule at equal CSS specificity won the cascade on source order alone. Caught immediately by the verification script rather than shipped and found later.

**Shipped:** v1.4.6 (Android 10406); sixteen suites passing.

## 62. One Root Cause, Five Widows, and a Border That Was Never Actually the Cascade's Fault (session 53, v1.4.7)

Same ask as §61, continued: keep auditing responsiveness and UX, but this time batch every fix and build every fork exactly once at the end instead of shipping per-finding. A 44-agent workflow swept visual layout quality (overlap, misalignment, spacing, orphan text, whitespace), configuration combinations (large-targets × orientation, high-contrast × theme, reduced-motion × the in-app Motion setting), desktop-specific behavior, and animation/motion consistency beyond the card flip — 35 candidates checked, 16 confirmed, every one independently re-verified by its own verify-phase agent before it reached this pass.

**Five orphan-text widows, one fix pattern.** `#/develop`'s "RANK ROADMAP — WHAT 'GOOD' LOOKS *LIKE*", `#/transition`'s "...Career *Focus*", `#/progress`'s "...a leader *DOES*", and `#/settings`'s "...or *DELETE*" all stranded a single word alone on a second line at 390px — a classic widow on prominent, isolated headings and one destructive-sounding button label. Four of five got a non-breaking space between the last two words; the fifth (`#/alc`'s "NCOPDS Drills — brief timer, essay rubric, MDMP") got the audit's own suggested trim instead, since the full label was long enough that an nbsp would have just moved the wrap point rather than closed it. Small, mechanical, and worth doing five times over rather than declaring "good enough" after the first — a widow reads as broken regardless of which heading it's on.

**One CSS oversight, three viewports, five sibling panels.** `#/money`'s BRS & TSP tab has five top-level `.panel` sections; four of the findings (390px, 834px, and 1440px all reproduced the identical pattern) were really one bug measured three times: `.fin-limits{margin:12px 0}` was the only one of three panel variants (`.panel`, `.fin-match`, `.fin-calc`) carrying an explicit margin, so two of the four panel-to-panel joins sat flush at 0px while the other two had comfortable 12px breathing room — the same generic component reading as two different spacing rhythms on one screen. Fixed by extending the same margin rule to all three variants.

**A flex-wrap bug found twice by the audit, a third time by not stopping at two.** `#/calendar`'s "What is next" list and `#/currency`'s "How current is this?" list both pair a title with a countdown/age badge in a `justify-content:space-between; flex-wrap:wrap` row — and on both routes, a short title kept its badge inline while a longer title pushed the *whole badge* onto its own line, reading as an orphaned second heading rather than a status pill. Same root cause on both routes, so before writing the fix a quick grep for the same inline-style shape elsewhere in the app turned up a third, unflagged instance in `src/app-modules/leader.js`'s overdue-soldier list — identical pattern, never audited, fixed the identical way. The fix itself: stop the container from wrapping as a whole, let the title (`flex:1 1 auto; min-width:0`) wrap its own text if it must, and pin the badge (`flex:0 0 auto; white-space:nowrap`) to the first line regardless of title length. This is the same lesson as §61's "same guarantee, two rules, drift apart if only one is in view" — except here it was the same bug typed three separate times by hand, not two rules diverging over time, and catching the third copy meant treating a confirmed pattern as a reason to search, not just a reason to fix what was already flagged.

**One out-of-taxonomy finding, kept anyway.** The audit was scoped to five categories, but flagged a sixth: `#/progress`'s LRM Balance radar chart clips 4 of its 6 axis labels at every viewport — "Develops (0%)" renders as "Develo", "Intellect (0%)" as ":t (0%)". Real, high-visibility, and outside the ask, which is exactly the right call to keep it rather than discard it on a technicality. Root cause: `text-anchor:start`/`end` labels on the diagonal axes were positioned by `pointFor(i, maxR+22)`, and the resulting x-coordinates simply ran past the SVG's own `0..260` viewBox before the text had finished rendering — worse at higher percentages, where an extra digit means more overhang. Fixed two ways at once rather than one large one: pulled the label ring in (`maxR+14`) *and* widened the viewBox horizontally (`±60px`), so every label has room even at "100%" on every axis, without touching the hexagon/grid geometry that was already correct.

**The high-contrast finding split into two different bugs wearing the same symptom.** Four findings, one description each ("border below 3:1 with High Contrast on"), but two unrelated root causes. Ten themes (sandstone-sun and nine others declared later in the file) had the exact §61 shape of bug: `html.hc { --border:var(--text-mute); }` and a same-specificity theme rule declared *after* it in source both set `--border`, so the later-declared theme silently won the cascade tie and the accessibility boost did nothing for any theme below that line — fixed with `!important`, which is the correct tool here specifically because the entire point of this rule is to win unconditionally regardless of theme. Blackout looked identical from the outside (2.63–2.70:1, still failing) but the `!important` fix did nothing for it, because blackout was declared *before* `html.hc` — it was never subject to the cascade bug at all. Its failure is a color problem: blackout compresses its whole palette into near-black and dark red, and even `--text-mute` (the boost target) barely lifts off blackout's own near-black surfaces. A verification run that still failed after the cascade fix landed is what surfaced the distinction — the fix had visibly worked (`--border` now correctly resolved to `--text-mute`, cascade-wise) and the number was still wrong, which is a different shape of bug than "the fix didn't apply." Routed blackout's hc-boosted border through `--text-dim` instead — already in the theme's own palette, ≥5.6:1 against both `--panel` and `--bg` — scoped to `html.hc[data-theme="blackout"]` so the other ten themes' fix is untouched.

**Reduced-motion zeroed durations but left delays standing.** The global `prefers-reduced-motion: reduce` rule (and its cinematic-specific echo) correctly floors every `animation-duration`/`transition-duration` to `.001ms`, but never touched `animation-delay` — so the Train scenario grid's staggered card entrance (`.05s`/`.1s`/`.15s`/`.2s` per item) still played out as a sequence of instant pops timed by leftover delay, on every keystroke of the search box, for a user who explicitly asked for reduced motion. Fixed by adding `animation-delay:0s !important` to all three reduced-motion rules — the OS-level media query, the manual in-app toggle's class rule, and the cinematic echo — rather than just the one the audit measured, since all three share the identical gap.

**Every fix independently re-verified against the rebuilt bundle** — not the audit's own numbers, a fresh Playwright pass per finding — and the process itself needed a second pass twice: the first high-contrast check silently never applied the theme (an un-awaited `G.theme.set()` raced a `page.reload()`), and the first settings-widow check used an arbitrary pixel-width threshold that failed a case that was actually already fixed. Both diagnosed by the same standing rule as every session since §51: an unexpected result means the tool is wrong before it means the app is wrong, but only after checking, never by assumption.

**Shipped:** v1.4.7 (Android 10407); sixteen suites passing.
## 63. The Open-Items List Was Lying, and So Were the Browser Dialogs (session 52, v1.4.5)

The ask was to work systematically through the project's own standing `openItems` list, then — mid-session — a redirect to make the UI more responsive and cleaner using real design tooling instead of more blind CSS review. Both threads converged on the same lesson: **verify the state of the world before trusting a cited number.**

**The first thing this session found was that its own starting point was wrong.** The `openItems` entry carried forward across dozens of sessions cited "~104 remaining contrast near-misses" from session 15. Session 19 — four sessions later — had already driven that exact theme set to zero, and nothing since had re-derived a real number; every later session either didn't mention it or half-quoted the stale figure. The deeper problem: `tools/test-contrast.mjs` was never a comprehensive sweep. It's a curated regression list of components previous sessions had specifically been bitten by (home/train probe, board, search, settings) — real coverage, but not the full 24-theme × 35-route surface anyone reading "contrast: all passed" would assume. Built `tools/test-contrast-full.mjs`: a genuine axe-core `color-contrast` pass over every route in every theme, 840 combinations, checked into `package.json` as `npm run test:contrast-full` so this exact failure mode — a stale number quoted for months because nobody re-ran the real check — can't recur.

**The first live run found 84 real violations, cleanly explained by exactly four root causes**, none of which any prior session had audited:

- **60 of the 84** were one bug: the Kiosk mode-picker's `.eyebrow`/`h3` text, wrapped in a native `<button>`. Browser UA stylesheets give `<button>` its own explicit `color` property — not `inherit` — so any custom button that doesn't set its own text color silently falls back to the browser default instead of the theme's `--text` token. Every *other* styled button in the app (`.btn`, `.nav button`, etc.) already sets its own color explicitly; this one component just never had to, until axe-core actually rendered it against all 24 themes' `--bg-2` values instead of the handful anyone had eyeballed. Fixed with one scoped rule: `button { color: inherit; }` — deliberately *not* the usual `font: inherit` pairing, since every styled button already sets its own font and there was no evidence-backed reason to take on that layout risk app-wide for a bug that was purely about color.
- **The other 24** were three separate instances of a failure mode this project has now fixed four separate times (`.fin-match-tbl th`, `.wr-anat-part`, `.res-url`): a raw accent (`--amber`, `--cyan`) used directly as text color against a themed background. Same resolution as §59's `.fin-h` fix — `var(--text)`/`var(--text-dim)`, not an `--ink-*` blend that's still unverified against all 24 themes for these specific contexts.

Re-swept after the fixes: 0/840. Full 27-suite `npm test` also stayed green throughout — the `button { color: inherit }` reset in particular was the kind of change that's cheap to get subtly wrong (global element selectors touch everything), so it got its own full-suite re-run before being trusted.

**Second finding, from the design-tooling redirect: the project's own design-debt doc was itself stale.** `GUIDON_DESIGN_SYSTEM.md` (§30-era, 2026-07-23) flagged native `confirm()`/`prompt()` as open debt, specifically calling out "Account panel uses them." A themed, accessible `G.modal` already existed (built ~session 44 per the file's own header comment) and had already replaced most call sites by the time this session started — searching the current source found only 8 remaining, and none of them in Account: board-drill schedule reset, progress reset, scenario delete, backup import, and three Money-worksheet "Clear all" buttons. All eight converted (danger-styled where the action is destructive), then live-verified through actual browser automation rather than trusted from a diff — modal opens, cancel/Escape/accept all confirmed working, the underlying action actually fires, the toast confirms it did. Zero native browser dialogs remain anywhere in the app (`alert()` had already been fully retired before this session).

**A tooling note for future sessions:** this environment's screenshot compositor never came up (the preview pane reported "not displayed, so the page is not compositing frames" on every attempt), and the configured dev-server harness failed silently on this repo's spaced Windows path. Neither blocked the work — `javascript_tool` for scripted interaction plus `read_page`/`get_page_text` for structural/content verification covered everything a pixel screenshot would have, and the standalone build (`dist/guidon-standalone.html`) opens directly via `file://` with no server needed at all, which turned out to be the more reliable path anyway.

**Shipped:** v1.4.5 (Android 10405); 27-suite `npm test` clean, plus the new 840-combination contrast sweep at zero, plus live-scripted verification of all 8 converted dialogs. releases/v1.4.5 supersedes v1.4.4.

**Still open from this session's own broader ask:** the ACFT reference contextual audit (~175 unaudited references), the civilian-education per-semester-hour rate (needs AR 600-8-19 §3-18 research), the acronyms dictionary refresh (2021 DoD baseline), the MOS Career Center FY26 shortage/SRB data refresh (perishable, needs live research), lazy-loading the ~5MB seed (flagged high-risk and deferred across many sessions already), and a screen-reader walk of the remaining unwalked flows. Windows code signing was explicitly *not* attempted — it requires a real Authenticode certificate purchase, a business/identity decision that isn't an agent's to make.

## 64. The ACFT Audit Session 42 Measured but Never Finished (session 53, v1.4.6)

Session 42 (§CLOSED entries in `openItems`) ran a contextual audit of ACFT references and explicitly stopped short: it measured 221 occurrences, hand-verified a handful, and left "175 not" audited on the record with a clear reason — "a regex cannot separate a current-standard claim from a historical one; needs a contextual pass." That contextual pass is what this session did.

**Scale management: extract, dedupe, delegate, then personally verify.** 159 string fields in `GUIDON_SEED` contained the literal text "ACFT" — but many were mirrored copies of the same underlying fact (a board question's `q`/`acceptableAnswer`/`boardAnswer`/`keyPoints` fields routinely repeat identical text), so the real unit of review was 104 distinct records. A sub-agent did the first-pass classification against an explicit four-bucket rubric (fine-historical / fine-neutral / stale-claim / needs-regulation-check), anchored to one piece of ground truth already established inside the app's own acronym dictionary: the AFT replaced the ACFT effective 1 June 2025. Before acting on any of it, the triage was spot-checked against raw source text for a sample spanning every bucket — this is board-exam and counseling-template content, and a wrong "fix" is worse than a stale-but-harmless mention.

**69 of 104 records were presenting a retired test as current**, over a year after it stopped being current. The pattern repeated across every content type in the app: a live counseling-bullet template ("a passing score on the record ACFT"), an IDP goal template ("Achieve a target ACFT score"), five training scenarios' actual dialogue and narration (an NCO saying "ACFT's coming up," a scenario summary reading "you just failed the ACFT — 340 total"), curriculum lessons listing "Military Training (awards, ACFT)" as a current promotion-points category, and board-question answers describing "ACFT standards" and "ACFT failure" with zero historical qualifier anywhere in the entry.

**The fix was a verified patch script, not a regex.** For each of the 69 records, the exact current field text was pulled from a fresh extraction of the live seed data, paired with a specific replacement, and applied only after confirming the expected occurrence count matched exactly (accounting for mirrored fields) — the script aborted with the file untouched if any count was wrong, rather than silently applying a partial or over-broad match. Two classes of field needed more care than a plain word swap: short/ambiguous values like a bare `"category": "ACFT"` that repeat verbatim across dozens of unrelated records (anchored to the specific record's `"id"` field instead of matched globally), and a handful of records needing real rewording rather than a substitution (the app's own transition-tracker entry, one doctrine entry explaining the ACFT→APFT succession, and a curriculum lesson that needed both the test name AND a stale training-priority item — Standing Power Throw, dropped from the test entirely in 2025 — replaced). 99 field-level replacements landed clean on the first fully-corrected run; the seed re-parsed to the same 1,014/3,629/336/163/182 record counts as before, confirming no records were added, removed, or corrupted — only text values changed.

**One fix surfaced a genuinely independent bug**, the same "reasoning caught what testing alone would have missed" pattern this project keeps re-learning: `doctrine.entries[200]` ("ACFT Alternate Aerobic Events," `asOf: 2022`, `confidence: verified`) listed the *old* test's alternate aerobic events — the 5,000-Meter Row, 12-Kilometer Bike, 800-Yard Swim — as current doctrine. Two other records in this same corpus, `board.questions[868]` and `[980]`, already correctly stated the AFT's real alternates: the 2.5-Mile Walk, 800-Yard Swim, 6.2-Mile Bicycle Ride. The app was answering the identical board question two different ways depending which page rendered it — structurally the same defect as session 42's taping-exemption contradiction, found by cross-referencing the corpus against itself rather than against any external source. Corrected the event list and retitled the entry to match.

**Two records were deliberately left untouched, and that decision matters as much as the 69 that were fixed.** `board.questions[59]` and `[311]` paraphrase AR 600-8-2 and AR 601-280 disqualifying-condition lists that name "APFT/ACFT failure" among several unrelated triggers. Whether the *current* (post-2025) text of either regulation has actually been updated to say AFT could not be confirmed with the tools available this session — a `WebSearch` came back with conflicting historical snippets, and a `WebFetch` against the live `armypubs.army.mil` AR 600-8-2 PDF failed to extract clean, readable text. Editing quoted-regulation board-answer content on a guess would be a worse outcome than leaving it stale; both are flagged in `GUIDON_STATE.json` as needing a human check against the actual current regulation text, not silently patched and not silently ignored.

**Verified:** full 27-suite `npm test` (0 fail), the seed re-parses with all record counts unchanged, and — the most direct confirmation available — `test-consistency.mjs`'s existing fitness-currency assertions ("no card teaches six events as the current test," "no card lists the Standing Power Throw or Leg Tuck as current," "no card gives 360 as the current minimum") now pass for a real reason rather than by omission, since this session is what actually made several of the cards those assertions cover correct.

**Shipped:** v1.4.6 (Android 10406). Housekeeping note: every intermediate analysis file this session generated (extraction dumps, triage reports, the patch script itself) was scratch work, not a permanent tool, and was deleted before commit — only `tools/test-contrast-full.mjs` from the prior session is meant to persist as checked-in tooling; this session's ACFT-audit scripts were one-shot and are gone.

## 65. Three More Checks — the Value Was in Knowing When Not to Fix (session 53 continued, v1.4.7)

Three remaining items from the open-items list, same session, same underlying discipline as §64’s regulation-check flags: research honestly, and when the evidence doesn't clear the bar, say so instead of guessing.

**Civilian-education per-semester-hour rate.** A prior session had already made the correct call here — the PPW's civilian-education points field takes a direct entry with a note that the per-credit rate "was not verified for this build, so the app does not guess it." This session tried to close that gap and couldn't: two independent research syntheses gave two different rates (1 point/semester-hour with explicit quarter/contact-hour conversion ratios, vs. 2 points/semester-hour), both attributed to the same regulation. The actual governing text — DA Pam 600-25's Point Scoring System section — never loaded cleanly from any source tried, including a direct fetch of the real DA Form 3355 PDF (which confirmed the app's existing 135/160 point caps but doesn't print the per-hour rate itself; that lives in the pamphlet's narrative text, not the worksheet form). The right move was the same one the prior session already made: don't compute a real Soldier's promotion points from a coin-flip. Left the field as designed, sharpened the disclaimer to name the specific conflict so the next attempt starts smarter instead of from zero.

**Acronyms dictionary, 2021 DoD baseline.** The honest scope check first: verifying 3,629 entries against a current external reference is not a same-session task under any circumstances, regardless of tooling. What was tractable: confirming the ACFT-related staleness that motivated this concern in the first place was already fixed by this session’s §64 audit, and a systematic internal sweep of every definition for known drift patterns (embedded years, dollar figures, IPPS-A references, self-asserted currency) — which found nothing beyond what was already caught. A genuinely newer DoD Dictionary edition exists (June 2025, four years past the app's baseline) and was located, but text extraction failed on every attempt — not because the document is unreadable, but because this sandbox lacks `pdftoppm`/poppler-utils, the same class of environment gap that blocked the AR 600-8-2 PDF in §64. The URL is on record in `GUIDON_STATE.json` now, which is the actual deliverable here: the next session that has working PDF tooling doesn't have to re-discover it.

**MOS Career Center FY26 shortage/SRB data.** This one inverted expectations. The section cites two specific document numbers — HQDA EXORD 117-26 and SRB MILPER 26-241 — which read exactly like the plausible-but-fabricated regulation citations that LLM-generated content sometimes produces under confident phrasing. Both turned out to be real, current, and hosted on the official `api.army.mil` domain; the MILPER is dated July 2026, weeks before this session ran. The shortage-MOS list and the overstrength direction both held up against independent research too. The section wasn't stale — it was better-sourced than assumed. The one thing that didn't hold up: the SRB note said "the maximum SRB will not exceed $180,000 combined-career cap," conflating two different figures — $180,000 is a per-award ceiling; a separate, higher lifetime cap exists (reported around $360,000 across independent sources) that this session couldn't confirm against primary text either, blocked by the same `api.army.mil` 403 pattern. Fixed the conflation without asserting the unconfirmed number as fact — the corrected text states what's known and explicitly flags what isn't.

**The pattern across all three:** research effort spent finding out something is already correct, or that a claimed fix can't be verified, is not wasted effort — it's the difference between "the app doesn't know" (honest, already true in two of these three cases) and "the app is confidently wrong" (the one failure mode a study aid can't afford). None of these three needed a big code change. All three needed someone to actually check.

**Verified:** full 27-suite `npm test` (0 fail) after each of the three changes (the disclaimer reword, no code change; the SRB note reword). v1.4.7 across index.html/package.json/tauri.conf.json/Android (versionCode 10407).

## 66. Board-Card Fact-Check — Pattern-Scanned, Then Personally Verified (session 53 continued, v1.4.8)

The open-items list still carried "board-drill study cards, never fact-checked" as a standing gap. This session closed the first pass of it: pattern-scan all 1,014 cards for risk indicators — dollar amounts, embedded years, explicit currency claims ("currently," "as of"), transition language, specific regulation citations — then personally verify every flagged candidate against a live source rather than trust the pattern match alone.

Most flags resolved exactly as expected: correctly-dated doctrine citations, or timeless content that happened to contain a number. A handful were real. The BAS rate was stale at $460/month (2024 figure) against a verified $476.95/month (the real annual COLA change landed since). SGLI premium was wrong in four places — one buried in a `boardAnswer` field a first read missed — and worse, two board cards didn't even agree with *each other* ($29 vs $30/month) before either was checked against reality; corrected all four to the verified current ~$33.50/month at max coverage. AR 670-1's cited edition date was wrong (2015 vs. the real 26 Jan 2021) in two cards. Tattoo-placement policy was genuinely out of date in two cards — newer-authorized areas (hand tattoo up to 1in beyond the ring-tattoo exception, back-of-neck up to 2in, behind-ear up to 1in) were missing — updated with an explicit "revised more than once, verify at board" hedge, since this is a policy area that evidently keeps moving.

Two officeholder facts got checked purely because they're the highest embarrassment-if-wrong content in the corpus, even with no specific reason to suspect either: Secretary of Defense Pete Hegseth and Sergeant Major of the Army Michael R. Weimer — both confirmed still correct. One lead was chased and correctly *not* acted on: a ponytail-policy card looked wrong against an initial search snippet, but fetching the actual source showed that snippet was describing a superseded interim policy — the card was already right. And one figure was confirmed current and deliberately left alone: HFP/IDP at $225/month (a proposed $450 increase exists, but isn't enacted).

**Verified:** full 27-suite `npm test` (0 fail). Along the way, an inline shell-escaping bug in my own verification command briefly suggested stale figures remained — re-verified with a proper script before trusting the scare, not after. v1.4.8 across index.html/package.json/tauri.conf.json/Android (versionCode 10408).

## 67. The Exhaustive Pass — Every One of 1,014 Cards, Read (session 53 continued, v1.4.9)

§66's pattern-scan was honest about its own limits: it could only catch what matched a risk pattern. This session asked the harder question directly — what's wrong in the cards that *don't* look suspicious? The only way to answer that is to read all of them, so that's what happened: all 1,014 cards split into 21 batches of ~50, grouped by category so related cards stayed together for cross-checking, each batch read in full by an independent reviewer with a three-way instruction — flag doctrinal errors you're confident about from training knowledge, flag internal contradictions against other cards on the same topic, and flag anything time-sensitive that needs a live check rather than a guess. Nineteen batches went to parallel sub-agents; two were reviewed personally after a sub-agent hit a content-filter block on SHARP/EO material — legitimate, mandatory professional Army training content, filtered anyway, and reframing the prompt didn't change the outcome, so the batches were read directly instead of forced through more retries.

225 raw findings came back. None were trusted on the reviewer's word alone — every one got cross-referenced against the rest of the corpus, checked against military-doctrine knowledge, or live-verified where the claim was genuinely time-sensitive. That second pass earned its keep twice over: one reviewer flagged a weapons-safety rule as wrong and proposed "fixing" it in the wrong direction — a live TC 3-22.9 check confirmed the card they wanted changed was actually the correct one, and the other card was the error. More uncomfortably, a fact this same session had already "fixed" earlier — the Army's three strategic roles — turned out to be wrong in a *different* way on the second look: current doctrine names four roles (Shape, Prevent, Prevail, Consolidate Gains), not the three it had just been corrected to. Caught and fixed within the same session rather than shipped wrong twice.

96 findings survived to become verified fixes. By area: leadership doctrine (mission command's six principles vs. a fabricated seventh; the direct/organizational leadership boundary; ADP 6-22's Intellect attribute is actually named "Expertise," not "Domain Knowledge"); military justice (AWOL vs. Desertion conflated on a 30-day line that's an administrative status, not a legal finding; a fabricated 2-year AWOL statute of limitations; a missing Article 15(a) vessel exception on a "cannot be denied" claim); pay and benefits (BRS TSP match arithmetic wrong in two places — 6% claimed against an actual 5%; SGLI premium wrong in four places and inconsistent with *itself* before it was even checked against reality, same failure shape as §66 but in different cards; the alternate 100% GI Bill eligibility path wrongly narrowed to "combat-related"); weapons and safety (three weapon control statuses, not four — "Cease Fire" is a fire command, not a status; the doctrinal 8-step cycle of functioning demoted to a parenthetical behind an informal 4-step version); and a systemic mislabel — fourteen separate SHARP cards all cited a regulation that doesn't exist, "AR 600-52," instead of the real AR 600-20 Chapter 7, with one card's own question naming the correct regulation while its own citation named the fake one.

Three leads were chased down and confirmed as non-issues rather than silently assumed fine: the EO protected-categories list (sexual orientation and gender identity are still current per the live AR 600-20 text, despite the app's political-administration context making this feel like a plausible place for drift), Military OneSource's session count and phone number (both current), and the ponytail-policy card from §66 (re-confirmed correct). One genuinely new time-sensitive catch: Fort Bragg was renamed Fort Liberty in 2023, then renamed back to Fort Bragg effective 7 March 2025 — a SERE-school card had this backwards, naming the post by its 2023–2025 name as if it were still current.

What's still open is open by design: of 225 raw findings, 96 became fixes, the rest split between confirmed non-issues, things checked and found already correct, and a residual ~130 lower-confidence or unverified items — narrow regulatory citations, exact-dollar or exact-day figures, and three NCO Creed quotes that don't match the verified creed text but weren't confident enough to guess a replacement for, rather than risk shipping a second wrong version of a verbatim quote. Nothing was silently dropped; the full list is preserved in this session's working notes for a future pass.

**Verified:** full 27-suite `npm test` (0 fail) after every patch round; the seed re-parses cleanly at every stage with all 1,014/3,629/336/163/182 record counts unchanged throughout — only text values changed, never record counts. Two JSON-breaking typos of my own (literal double-quotes embedded inside replacement text) were caught and fixed before either reached a build. v1.4.9 across index.html/package.json/tauri.conf.json/Android (versionCode 10409).

## 68. Reconciling Two Parallel Session Threads (v1.4.10)

Sections 60–62 and 63–67 above both descend from the same §59 (v1.4.4) but ran as two independent session threads on two diverged branches — one continuing directly on `main` (§60–62: the quota-interrupted contrast sweep, the responsive/orientation audit, the UX/config-combo batch), the other on a separate feature branch (§63–67: the open-items systematic pass, the ACFT contextual audit, the three research-backed checks, and the two board-card fact-check passes). Neither thread was aware of the other while it ran. This section is the reconciliation.

**What actually conflicted, checked structurally rather than assumed:** the two branches' `index.html` changes were diffed against their common `v1.4.4` ancestor with the embedded `GUIDON_SEED` JSON isolated from the surrounding CSS/JS (that JSON lives on a single ~3.2MB line in the file, which makes a line-based diff meaningless for judging the size of a change inside it — a one-character edit and a five-hundred-field edit both show as "1 line changed"). That isolation showed: `main`'s three sessions never touched the seed content at all (one single incidental non-breaking-space byte aside, an editor artifact, not a real edit) — every one of `main`'s fixes lived in CSS, in the standalone SVG radar-chart geometry, or in the separately-built `src/app-modules/*.js` files this branch never touched. This branch's fixes were the reverse: entirely inside the seed content (93 board cards, 96 field-level fixes), plus a handful of CSS fixes of its own. Where both branches fixed CSS, the actual overlap was three small hunks, not a true conflict in any of them: one identical fix landing on both sides independently (`.fin-match-tbl th`, the same raw-accent-as-text bug found by both audits), and two complementary fixes to different nearby selectors (`main` fixed `.wr-grader-h` and `.fin-out-cell`; this branch fixed `.wr-anat-part`) that only *looked* like they might collide because they sat in the same CSS block.

**The merge itself:** `git merge origin/main` reported conflicts in exactly the seven files both branches had touched — the three canonical docs, the three version-bump files, and `index.html`. Six of those seven resolved by inspection alone (docs: keep both threads' entries, renumbered where section numbers collided; version files: bump past both to 1.4.10 rather than pick one side's number arbitrarily). `index.html` itself needed exactly two real resolutions: the `GUIDON_APP_VERSION` constant (same treatment, 1.4.10) and the `GUIDON_SEED` line — resolved by taking this branch's copy whole, since the structural check above confirmed `main` had made no real edits to it to lose.

**Verified:** full 27-suite `npm test` (0 fail) on the merged tree; the seed re-parses cleanly with all 1,014/3,629/336/163/182 record counts unchanged. Every one of `main`'s CSS/JS/app-module fixes and every one of this branch's 96 board-card fixes are both present in the merged file — checked by re-running the same structural diff against the merge result, not assumed from the merge summary. Shipped as v1.4.10 across index.html/package.json/tauri.conf.json/Android (versionCode 10410).

## 69. Four Commits, No Version Bump — Closing a Gap in the Project's Own Discipline (session 53 continued, v1.4.11)

§68 closed out the two parallel threads and reset to a single line at v1.4.10. Four more commits landed on that line the same day — a same-day recommendations batch, a Home redesign, an SFC+ addition, and Android notifications — and every one of the discipline habits this file has documented for 68 sections running (version bump, changelog entry, this file) quietly stopped happening. Not a design decision, just momentum: each piece of work got its own real verification and shipped, and the bookkeeping step at the end of each got skipped in favor of moving to the next thing. This section is that bookkeeping, done once, for all four.

**The recommendations batch found one real bug wearing a "just tidying up" disguise.** The Data & Backup panel in Settings had its own separate export/import implementation that only ever round-tripped `{settings, attempts}` — silently dropping profile, board-date, SRS scheduling, IDP goals, and counseling drafts on every export from that panel, while the Account page's backup button (calling the correct `G.backup.exportAll()`/`importAll()` the whole time) sat one page away doing it right. Same lesson this file has documented before under a different name (§61's "same guarantee, two rules, drift apart if only one is in view") — except this time the two implementations weren't even solving the same problem correctly; one was just wrong. Alongside it: a static lint pass (`tools/lint-patterns.mjs`) for the three bug shapes this project's own history shows repeating — raw-accent-as-text-color, touch-target media-query drift, the flex-wrap badge bug — wired ahead of the build so none of the three can reach one silently again; a privacy policy grounded in actually reading the CSP/privacy test files rather than assuming their coverage; and a parallel test harness that cut 15 independent Playwright suites from serial to concurrent execution.

**The Home redesign is the same "computed twice" bug this project keeps finding, just in JavaScript instead of CSS this time.** Two sections of `views.home` independently looped the full board-question bank to arrive at the same due-card count — one more instance of a number existing in two places instead of one, the exact failure shape §61 and this session's own backup bug both share, just with duplicated logic instead of duplicated CSS. Thirteen stacked sections collapsed into one hero card built against an approved mockup before any code was touched, absorbing the readiness headline, the (now singly-computed) due count, and the recommended-next scenario into the first thing a user sees instead of the last.

**The SFC+ explainer is this session's version of a pattern §64–§67 established repeatedly: research first, and let what you find override what you already assumed.** GUIDON's promotion-points worksheet had only ever modeled semi-centralized SGT/SSG boards; nothing existed for the point-free, merit-ranked centralized process SFC and above actually go through. Live research before writing the explainer caught a wrong assumption in the plan itself — DA Photos have been out of centralized board files Army-wide since August 2020, specifically to reduce bias — so the correction leads the feature instead of shipping inside a guess. The explainer that resulted holds itself to the same no-invented-numbers discipline the SGT/SSG worksheet already does: comparison, correction, a hedged and cited description of file contents, a pointer to the real source. A second half of the same plan — rebuilding a Forms Trainer NCOER practice form — turned out to already be done by the independent parallel session that ran during §60–§67; checking before building avoided duplicating work that already existed.

**Notifications closes the list's last item, and the verification for it went further than the code review that shipped it.** The Reminders editor's own copy had said "no notifications are sent" since the feature existed; `src/notify.js` makes that capability real and opt-in, using the exact Capacitor-detection convention `native.js` established — inert everywhere except a real Android build, permission requested from exactly one Settings toggle, never at launch. What makes this section worth writing, though, isn't the feature — it's what closing it required that no prior notification-adjacent work in this file has needed: a real device, twice. A mocked-native Playwright pass can confirm the JavaScript calls `requestPermissions()` and `schedule()` correctly; it cannot confirm Android actually shows a user the system dialog, actually grants `POST_NOTIFICATIONS`, or actually registers a wakeable `RTC` alarm. All three got checked directly rather than assumed from the mock — on both the Fold and the Tab, `dumpsys package` showing `granted=true` and `dumpsys alarm` showing a live `TimedNotificationPublisher` entry at the reminder's date, then `Reason=alarm_cancelled` in the alarm log after removing it. And once merged, "does the device actually have this code" got checked the same way this file's very first sections checked structural claims — not by version label (frozen at 1.4.10 on both devices' Settings screen the whole time, telling you nothing) but by pulling each device's installed `base.apk` and hash-comparing its bundled `index.html`/`sw.js` byte-for-byte against a fresh build of `main`. Both matched exactly, on both devices.

**Verified:** each of the four pieces of work verified independently at the time it shipped (lint/build/test-suite passes documented in their own commits); this section's own contribution — the two-device real-permission-and-alarm walk and the two-device hash-verified sync check — run fresh rather than re-cited from memory. v1.4.11 across index.html/package.json/tauri.conf.json/Android (versionCode 10411).

## 70. An Audit, A Roadmap, and Week One of It — Quiz Motion Plus Two P0s (session 53 continued, v1.4.12)

Asked directly to "map out planned upgrades for the next 3 weeks," a 4-agent audit read the app fresh across motion/gesture, content staleness, UX polish, and tech debt — each briefed to read this file's own history first, specifically so it would surface what 69 prior sections *hadn't* caught rather than re-litigate settled ground. It found 31 raw findings and two real P0s. A fifth agent synthesized all of it into a 3-week roadmap. This section is week one of that roadmap, plus a sixth piece of work asked for in the same breath: give Quiz mode's cards the same quality of motion Board Drill's flip already has.

**Quiz mode's gap turned out to be total, not partial.** §68's own reconciliation and every session since have treated the flip card (§-many, going back to its "Quizlet-style" origin) as the app's motion showpiece — real 3D `perspective`/`rotateY`, three tuned motion-level presets, a `will-change` promoted before the first frame specifically to avoid a rasterization stutter. Reading Quiz mode's actual renderer (not assuming it shared the flip card's machinery just because both live under Board Prep) found it had none of that: multiple-choice buttons, an instant class-toggle for correct/wrong, no transition anywhere. The fix isn't a literal flip — multiple choice has no back face to reveal — it's the same care applied to what Quiz mode actually is: a card entrance, a correct-pulse, a wrong-shake, a feedback line that fades in only once it has real content (not a keyframe that already finished on an empty div, which the first version of this fix got wrong before catching it with a screenshot-timed check), and an exit-then-advance on Next instead of an instant swap. New `--quiz-dur`/`--quiz-ease` tokens hang off the same `data-motion` levels rather than inventing their own scale.

**The reduced-motion check for the exit-wait had to be done in JS, not read off the CSS.** The natural-looking approach — read `--quiz-dur`'s computed value and `setTimeout` for that long — is wrong in exactly the case that matters most: under `data-motion="minimal"` or OS-level `prefers-reduced-motion`, the CSS custom property still reports its normal string value (".32s") even though the actual `transition-duration` is force-collapsed to `.001ms` by this file's existing global kill-switch (`html.reduce-motion *`, `@media (prefers-reduced-motion: reduce)`). Trusting the variable would have shipped a real accessibility regression: a UI that claims to respect reduced motion while still making a screen-reader or vestibular-disorder user wait out a fake 320ms delay on every single answer. Checked `data-motion` and `matchMedia` directly instead, and confirmed the fix by timing the actual advance rather than reading the code and assuming it was right — 4-5ms under both conditions, not the ~320ms it would have been on the wrong approach.

**Records Readiness was a P0 hiding behind working-looking code.** `records.js` built each checklist label with `{ htmlFor: id }` — correct-looking JSX-convention naming, wrong for this file's `el()` helper, which writes unrecognized keys via raw `setAttribute` and so wrote a dead `htmlfor` attribute the browser never binds to anything. The result: tapping the label text on any of 23 rows on the one screen literally named "Records Readiness" did nothing, and only the bare 13px native checkbox — smaller than every touch-target guideline in this app's own CSS — actually worked. Live-confirmed with Playwright before and after: the exact click that used to do nothing now toggles the box. Same session, added checkboxes to the coarse-pointer touch-target treatment they'd been missing from since that list existed — scaled the same way `.toggle` already is, since `min-height` (what every other control in that list uses) does nothing for a native checkbox's rendered size.

**SGLI went stale again, one day after being "fixed."** v1.4.8/v1.4.9 verified the premium at ~$33.50/month on 2026-08-08; this audit, run the very next day, found that verification was itself against a rate that had already been superseded a year earlier — the real cut to $0.05/$1,000 + $1 TSGLI took effect 1 Jul 2025, well before either of those sessions ran. The correct figure is $26.00/month, not $33.50. Worth naming plainly: a careful fact-check session can still ship wrong content if the source it checks against is itself stale, and the only defense is checking again, not checking harder once. One curriculum lesson (`jsf-l4`) had carried two *different* wrong figures in its own two difficulty tiers — $33.50 and $29 — that had never agreed with each other even before either was compared to reality, the same "two rules, one guarantee, drifted apart" shape this file has named under other names since §61.

**The confirm dialog and PDF filler both got real motion, off one shared vocabulary despite being two separate implementations.** `modal.js`'s `dialog()` and the PDF filler's `util.modalTrap` never shared code — one is a hand-rolled promise-based confirm, the other a reusable focus-trap helper with exactly one consumer today. Rather than build two different motion systems, both now toggle the identical `.modaltrap-open` class and use the identical `requestAnimationFrame`-gated open / `transitionend`-aware close pattern, so a future modal adopting either mechanism inherits working motion for free. `#toast` had this right for years; the app's two actual dialog surfaces — used for every destructive confirm and the DA 4856 filler — never had.

**The full contrast sweep finally runs where it matters.** `tools/test-contrast-full.mjs` — 840 real axe-core checks across 24 themes × 35 routes — has existed as checked-in tooling but only ran on manual invocation; the wired-in `npm test` battery used a 9-surface abbreviated version instead. Running the full sweep to wire it in surfaced a genuine violation neither the abbreviated suite nor `lint-patterns.mjs`'s baseline-tolerant raw-accent-as-text check had ever caught: `.tag.leads` at 4.46:1 in desert-cadence, one theme out of 24, just under the 4.5:1 line. Fixed with the same `--ink-amber` blend token this exact bug shape gets everywhere else in the file. This is the "presence is not correctness" lesson this file's own STANDING notes have named twice, now closed for this specific safety net rather than left as a note about it.

**Verified:** each fix confirmed by a dedicated Playwright pass built for that specific fix — not the general suite alone, and not a visual impression. The Quiz motion and modal motion passes specifically confirmed animation state via computed styles (`animationName`, `opacity`, class presence) at each stage of the interaction, not just "did it look right in a screenshot." Full `npm test` (15/15, including the now-default full contrast sweep) green after every individual change lands, the same discipline this file's history has held to since it started measuring rather than assuming. v1.4.12 across index.html/package.json/tauri.conf.json/Android (versionCode 10412).

## 71. Week Two — the Motion System, and a Regression the Suite Caught Before This Section Was Written (session 53 continued, v1.4.13)

Week two of the roadmap §70 shipped week one of: the motion-system work, the tab-strip consolidation, the Board Drill hint trim, and structured review-date tracking for perishable content. The section worth reading closely is the tab-strip one — not because the feature matters most, but because of what happened while shipping it.

**Fill bars, scoped honestly rather than exhaustively.** The roadmap's own audit named ~10 selectors animating `width`/`height` directly; the actual file had 13, and reading each one's real usage mattered more than the count. Two — `.qz-progress-fill`, `.fin-budget-bar`, `.tr-prog-fill`, `.eng-prog-fill`, `.idp-dash-fill`/`.idp-dash-mini-fill`, `.promo-bar-fill` — are named, semantic, individually-tuned components; converted all of them to `transform:scaleX()`. The generic `.bar > span` pattern, used ad hoc in some fifteen unrelated places across the app, was deliberately left alone: most of those set their width once at element creation via the `style:` attribute and never trigger a transition again in the element's lifetime, so `transition:width` on them is dead weight, not a live performance cost — converting all fifteen would have been effort spent on a problem that mostly doesn't actually occur, at real risk to fifteen different features. The one fill bar with a genuinely live, repeatedly-reassigned case — `promo-bar-fill`, recalculated on every drag of a promotion-points slider — got the most scrutiny of the batch: verified the `transform` value actually changes on input (`matrix(0.21, 0, 0, 1, 0, 0)`), not just that the CSS property changed from `width` to `transform` in the source.

**The nav accordion joins a pattern instead of staying the one exception to it.** Four other accordions in this file (§-many sessions ago, the skill-tree and both Resources ones) already share one CSS mechanism for a rotating chevron: a single icon, rotated via `[aria-expanded="true"] .gi { transform: rotate(90deg) }`, never swapped for a different glyph. `renderNav()` alone did something else — two different chevron icons swapped in and out, and worse, the entire sidebar torn down and rebuilt via `util.clear(navEl)` on every single group toggle, the single most-repeated interaction on the app's own most-traveled UI surface. Fixed by extending the *same* shared selector to include `.nav-group-chev`, and by restructuring `renderNav()` so a group's buttons render into a `.nav-group-body` wrapper that exists in the DOM whether the group is open or closed — toggling became a class change on nodes that already exist, not a rebuild. Verified this precisely: tagged the header and body nodes with a marker attribute before toggling, then confirmed after toggling (twice, both directions) that the *same* nodes were still there, not new ones with the same look.

**The shared motion-token pair stayed small on purpose.** `--dur-base`/`--ease-standard`, added to the same three `data-motion` blocks that already carry `--qz-flip-*`, `--quiz-*`, and `--modal-*` from the last two sections — but migrated onto exactly one thing, the nav accordion's new transition, because that was the transition actually created this session with nowhere established to live. The fill bars kept their own hand-tuned per-component durations rather than being flattened onto the new generic pair: a slider a user is actively dragging and a dashboard bar that renders once are allowed to feel different, and forcing them to the same timing would have been a regression dressed up as consistency.

**The swipe dead-zone is eight pixels of nothing, verified as eight pixels of nothing.** `wireSwipe()`'s touchmove handler had no minimum-movement threshold before it started visually dragging the flipped card — only the unrelated 70px release threshold, which decides the *grade*, not whether the card *moves*. Verified with synthetic `TouchEvent` dispatch (CDP-level touch injection proved unreliable in this environment and was abandoned in favor of dispatching real `Touch`/`TouchEvent` objects directly at the card): 5px of movement now touches nothing on the card's inline style at all; 20px engages the drag exactly as it did before this session.

**Unifying the tab strips is the section that mattered most, because of what it caught about itself.** Two implementations solving one problem — the shared `.tabbar`/`util.tabBtn()` (seven consumers) that wrapped onto as many rows as needed, and Risk Management's own hand-rolled `.tab-row`/`.tabBtn` that scrolled instead — got consolidated onto the scrolling behavior, since wrapping was the actual, demonstrated bug (Money's eight tabs, four full rows, before any content appeared on a phone). Migrating `.tabbar` from `flex-wrap:wrap` to `flex-wrap:nowrap; overflow-x:auto` should have been the easy half of this section. It introduced a real regression instead: a nowrap flex row's un-shrinkable content contributes its full min-content width to its ancestors regardless of its own `overflow-x:auto`, and on `#/money` specifically — the one route with eight tabs, the widest tab strip in the app — this pushed `.main` sixteen pixels past its own CSS Grid track at a 1152px viewport, genuine page-level horizontal overflow. This shipped nowhere: the very next full `npm test` run caught it on its own, unprompted, as `tabS9-landscape overflow @#/money doc=16 wide=0` — `verify.mjs`'s responsive sweep doing exactly the job it was built for. Two fixes were tried and measured before the right one was found: `min-width:0` on the tabbar itself did nothing (confirmed by re-measuring, not assumed fixed); `min-width:0` on `.main` also did nothing. What worked, and was verified against a clean bisect against the last commit (the failure was reproducibly absent before this session's tabbar edit and reproducibly present after, checked directly via `git stash` rather than inferred): `width:0; min-width:100%` on `.tabbar`, which removes the content-driven sizing entirely before re-expanding the element to its actual container's width — the tabs then scroll inside a size fixed to their parent, never contributing their own size upward again. Re-ran the full responsive sweep clean (27/27) after the fix, not just the one failing case.

**Verified:** every item this section covers has its own dedicated Playwright pass, run and passing before moving to the next item — the same discipline as every prior week. The one place that discipline paid for itself directly: the tab-strip regression was caught by the project's own general-purpose safety net (`verify.mjs`), not a bespoke check written for this specific change, which is the whole reason that sweep exists as a standing part of `npm test` rather than a one-off. A single `test:privacy` failure during a later parallel run was confirmed transient by running it standalone (clean) before being set aside, rather than either ignored or over-investigated. v1.4.13 across index.html/package.json/tauri.conf.json/Android (versionCode 10413).
