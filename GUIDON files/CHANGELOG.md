...
**`G.engine.run()`'s solo (non-Collective) path now reports real completion state** (`src/index.html`, the engine module). Investigating ROADMAP item H ("a real `G.engine.run()` completion contract; Collective mode as an engine render mode; Board Simulator on a phase registry") found two of its three bullets already substantively shipped and the roadmap text simply never updated: Collective mode has existed since v1.12.0 (`src/app-modules/00-roadmap-bootstrap.js`'s `runCollective`/`renderDecision`/`finish()`, wrapping the public `G.engine.run` API), and Board Simulator (`src/app-modules/mock-board-simulator.js`) already runs off a real, data-driven `STEPS` registry, not hardcoded per-step branches. The one genuine gap: Collective's wrapper calls `onExit({completed, cancelled, scenarioId})` on every real exit, but the underlying solo engine - used by `#/train`, Board Simulator's own steps 1 and 3, Counsel training, and everywhere else - called `onExit()` with no arguments at all, at all three of its real exit points (the persistent header's Exit button, a training module's pass/fail outcome Exit, and the final AAR's Done button). Board Simulator's own `scenarioStepFinished()` was already written to check `result.completed === true` first - it just never received that signal, so it always fell through to its attempt-log side-channel workaround (`finishedRunSince()`) for every solo-mode step. Fixed by making the solo engine pass the identical `{completed, cancelled, scenarioId}` shape Collective already uses, at all three exit points (`completed:true` at the two outcome-screen exits, `completed:false`/`cancelled:true` at the mid-play header Exit) - now the reference contract both implementations share, documented at `G.engine.run()`'s own definition. The attempt-log fallback stays: neither this fix nor Collective's own callback can ever fire for the fourth, honest way a session ends - navigating away by the nav bar or back button with no callback at all - so that case still needs its own signal.
