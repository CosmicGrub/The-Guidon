...
## 2026-09-22 - v1.15.1: ROADMAP item I, the remaining third (#/drills wired to `store.prtMeta()`)

**`#/drills`' Leadership Drills PRT session builder** (`src/index.html`, `prtDrill()`). Preparation Drill's ten exercise names were a hand-maintained literal array that happened to match `window.GUIDON_SEED.prt.drills`'s own verified "pd" record - the exact kind of duplication a prior roadmap-audit round already caught quietly drifting out of sync once (see `tools/test-drills-prt-sourcing.mjs`'s own header comment). Now reads through `G.store.prtMeta()`, the same accessor `#/prt` and PT Planner already use, with the literal array kept only as a fallback for the seed-not-loaded case. The other seven blocks (SSD/CD1/CD2/HSD/MMD1/MMD2/RD) have no equivalent seed record to read from at all - `window.GUIDON_SEED.prt.drills` only carries "pd" - so they're unchanged, still hand-maintained, still correctly tagged "(content pending)". No visible behavior change; `tools/test-drills-prt-sourcing.mjs` and `tools/test-prt.mjs` both pass unmodified. Completes ROADMAP 3g item I (PT Planner and `#/prt` were unified in v1.13.0; this was the deferred third).
